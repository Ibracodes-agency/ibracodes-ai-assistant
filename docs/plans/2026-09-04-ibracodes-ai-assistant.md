...
- Keep the refusal paragraph but make its offer fit the site: without commerce it offers to answer a question about the site or reach a human, drops the "what the shop stocks" sentence, and every "shop" in the security, earlier-turns and no-handoff lines reads "site". Wrap the "Grounding" product paragraph, the "Product cards" paragraph and the "You cannot place orders" paragraph in `if (Capabilities::has_commerce())`. The `ask_first` and `price_policy` paragraphs also only with commerce.
