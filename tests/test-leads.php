...
$own = static function (array $result): array {
    if (! empty($result['id'])) {
        $GLOBALS['ibraai_test_leads'][] = (int) $result['id'];
    }
...
global $wpdb;
$table = $wpdb->prefix . 'ibraai_leads';
ibraai_assert_same($table, $wpdb->get_var("SHOW TABLES LIKE '{$table}'"), 'leads table exists');
...
ibraai_assert_same(1, (int) $row['email_sent'], 'email_sent set once the owner was reached');
...
ibraai_assert_same(sprintf(__('New lead from the AI Assistant: %s', 'ibracodes-ai-assistant'), 'Dana Levi'), $sent[0]['subject'], 'a first capture is announced as new');
...
ibraai_assert_same(sprintf(__('Updated lead from the AI Assistant: %s', 'ibracodes-ai-assistant'), 'Dana Levi'), $sent[1]['subject'], 'a corrected lead is announced as updated');

$rows = Leads::list(1, 20);
ibraai_assert($rows['total'] >= 1, 'listing works');
foreach (['%', '_'] as $needle) {
    $found = Leads::list(1, 20, $needle);
    ibraai_assert_same('', $wpdb->last_error, "list() search with {$needle} runs without an SQL error");
    ibraai_assert(is_int($found['total']) && is_array($found['rows']), "list() search with {$needle} returns the usual shape");
}
...
ibraai_assert_same(true, $rtl['saved'] ?? false, 'direction marks around a phone number do not reject it');
ibraai_assert_same('050-123-4567', Leads::find((int) $rtl['id'])['contact'], 'direction marks are not stored');
...
foreach ([
    ...
    ibraai_assert_same($expected, Leads::is_phone($value), 'is_phone(' . json_encode($value) . ')');
}

// a failed email keeps the lead and leaves the flag off for the admin to show
$GLOBALS['ibraai_mail_ok'] = false;
$unsent = $own(Leads::capture(['name' => 'Quiet Lead', 'contact' => 'quiet@example.com', 'request' => 'x'], 0, 0));
ibraai_assert_same(true, $unsent['saved'] ?? false, 'a failed email keeps the lead');
ibraai_assert_same(0, (int) Leads::find((int) $unsent['id'])['email_sent'], 'a failed email leaves email_sent 0');
$GLOBALS['ibraai_mail_ok'] = true;
...
Leads::purge();
ibraai_assert_same(null, Leads::find((int) $old['id']), 'purge deletes a lead past the retention window');
ibraai_assert(Leads::find((int) $fresh['id']) !== null, 'purge keeps a fresh lead');
...
$messages = $wpdb->prefix . 'ibraai_messages';
$thread = $start_thread();
ibraai_assert(DB::thread($thread) !== null && count(DB::thread($thread)['messages']) === 2, 'fixture thread has a transcript');
$linked = $own(Leads::capture(['name' => 'Linked Lead', 'contact' => '0503333333', 'request' => 'x'], $thread, 0));
ibraai_assert_same($thread, (int) Leads::find((int) $linked['id'])['thread_id'], 'lead points at the thread');
Leads::delete((int) $linked['id']);
ibraai_assert_same(null, DB::thread($thread), 'deleting the lead deletes its thread');
ibraai_assert_same(0, (int) $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM {$messages} WHERE thread_id = %d", $thread)), 'deleting the lead deletes the thread\'s messages');
$thread = $start_thread();
...
Leads::purge();
ibraai_assert_same(null, Leads::find((int) $expired['id']), 'purge removed the expired lead');
ibraai_assert_same(null, DB::thread($thread), 'purging the lead deletes its thread');
ibraai_assert_same(0, (int) $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM {$messages} WHERE thread_id = %d", $thread)), 'purging the lead deletes the thread\'s messages');

$cards = [];
$via_tool = $own(Tools::run('capture_lead', ['name' => 'Noa', 'contact' => 'noa@example.com', 'request' => 'call me'], $cards, ['thread_id' => 78, 'page_id' => 0]));
ibraai_assert_same(true, $via_tool['saved'] ?? false, 'the tool path saves a lead');
ibraai_assert_same(78, (int) Leads::find((int) $via_tool['id'])['thread_id'], 'the tool path records the thread from the context');
Leads::delete((int) $via_tool['id']);

Settings::update(['leads_enabled' => false, 'leads_when' => '']);
ibraai_assert_same(['error' => 'unknown_tool'], Tools::run('capture_lead', ['name' => 'Noa', 'contact' => 'noa@example.com', 'request' => 'call me'], $cards, ['thread_id' => 78, 'page_id' => 0]), 'the tool refuses while leads are off');
ibraai_assert_same($rows_before, (int) $wpdb->get_var("SELECT COUNT(*) FROM {$table}"), 'no lead rows left behind');
