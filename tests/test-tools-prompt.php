...
wsa_assert(! str_contains($prompt, 'find a product') && ! str_contains($prompt, 'shop stocks') && ! str_contains($prompt, 'outside the shop'), 'no shop wording without WooCommerce');
wsa_assert(str_contains($prompt, 'answer a question about this site or to reach a human'), 'the refusal offers the site instead of products');
wsa_assert(str_contains($prompt, "Site facts, the only information beyond the site content you may state as fact:\nOpen Sunday to Thursday."), 'facts labelled as site facts without WooCommerce');
Settings::update(['handoff_url' => '']);
wsa_assert(str_contains(Prompt::system_message()['content'], "suggest the site's contact page"), 'the no-button line names the site without WooCommerce');
...
wsa_assert(str_contains($prompt, 'Store facts, the only non-catalog information'), 'facts labelled as store facts with WooCommerce');
wsa_assert(str_contains($prompt, 'shop stocks') && str_contains($prompt, 'outside the shop'), 'shop wording kept with WooCommerce');
wsa_assert(str_contains($prompt, 'summing up what a page on this site says is part of your job'), 'the refusal leaves the site\'s own pages alone');

Settings::update(['leads_enabled' => true, 'leads_when' => 'when someone asks for a demo']);
$prompt = Prompt::system_message()['content'];
wsa_assert(str_contains($prompt, 'Lead capture:') && str_contains($prompt, 'when someone asks for a demo'), 'lead paragraph carries the owner\'s line');
wsa_assert(str_contains($prompt, 'offer it before the contact option'), 'lead capture comes before the contact option when both are on');
Settings::update(['handoff_url' => '']);
wsa_assert(! str_contains(Prompt::system_message()['content'], 'offer it before the contact option'), 'no precedence sentence without a contact option');
...
wsa_assert(str_contains($prompt, Prompt::PAGE_OPEN . "\nThe gold package") && str_contains($prompt, "installation.\n" . Prompt::PAGE_CLOSE), 'current page text sits between the page markers');
wsa_assert(str_contains($prompt, 'Text between a ' . Prompt::PAGE_OPEN . ' line and a ' . Prompt::PAGE_CLOSE . ' line'), 'the content rule names the markers');
...
wsa_assert(str_starts_with($out['results'][0]['passage'], Prompt::PAGE_OPEN . "\n") && str_ends_with($out['results'][0]['passage'], "\n" . Prompt::PAGE_CLOSE), 'passages are delimited');
$one = Tools::run('get_page', ['id' => $page], $cards);
wsa_assert(str_contains($one['text'], 'installation'), 'get_page returns the text');
wsa_assert(str_starts_with($one['text'], Prompt::PAGE_OPEN . "\n") && str_ends_with($one['text'], "\n" . Prompt::PAGE_CLOSE), 'get_page text is delimited');
$amp = wsa_make_post('Terms & Conditions', 'The terms apply.');
wsa_assert_same('Terms & Conditions', Tools::run('get_page', ['id' => $amp], $cards)['title'], 'get_page decodes the title');
...
wsa_assert_same(['error' => 'not_found'], Tools::run('get_page', ['id' => $page], $cards), 'get_page refuses an excluded id');
remove_filter('wsa_content_excluded_ids', $exclude);
$private = wsa_make_post('Hidden', 'xq-private-secret', 'page', 'private');
wsa_assert_same(['error' => 'not_found'], Tools::run('get_page', ['id' => $private], $cards), 'get_page refuses a private post');
$locked = wsa_make_post('Locked', 'xq-locked-secret', 'page', 'publish', ['post_password' => 'pw']);
wsa_assert_same(['error' => 'not_found'], Tools::run('get_page', ['id' => $locked], $cards), 'get_page refuses a password-protected post');
